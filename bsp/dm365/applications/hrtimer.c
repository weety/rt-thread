...
#ifdef RT_HRTIMER_LIST
static struct rt_hrtimer_head hrtimer_queue = {RT_LIST_OBJECT_INIT(hrtimer_queue.head), RT_NULL, RT_NULL, RT_NULL};
#else
...
#ifdef RT_HRTIMER_LIST
	rt_list_t *l;
	struct rt_hrtimer *pos;

	for (l = head->head.next; l != &head->head; l = l->next)
	{
		pos = rt_list_entry(l, struct rt_hrtimer, node);
		if (node->deadline < pos->deadline)
		{
			rt_list_insert_before(&pos->node, &node->node);
			break;
		}
	}

	if (l == &head->head)
		rt_list_insert_after(l, &node->node);

	head->next = rt_list_entry(head->head.next, struct rt_hrtimer, node);
...
static void rt_hrtimer_invoke(void)
{
	...
	while (node = hrtimer_queue.next)
	{
		/* get the current time */
		delta = node->deadline - rt_clocksource_absolute_time();

		if (delta >= event->min_delta_ns)
		{
			break;
		}
	...
	cycle = ((unsigned long long) delta * event->mult) >> event->shift;
...
uint64_t rt_clocks_calc_max_nsecs(rt_uint32_t mult, rt_uint32_t shift, rt_uint32_t maxadj, uint64_t mask, uint64_t *max_cyc)
{
	...
	max_nsecs = (max_cycles * (mult - maxadj)) >> shift;
...
rt_ktime_t rt_clocksource_absolute_time(void)
{
	...
	ns_offset = (cycle_delta * cs->mult) >> cs->shift;
